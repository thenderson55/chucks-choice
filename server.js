  ...
  try {
     await axios({
      ...
      try {
         await axios({
          method:"GET",
          url:`https://dark-sky.p.rapidapi.com/${resp.data.location.lat},${resp.data.location.lng}?lang=en&units=auto`,
          headers:{
          ...
          try {
             await axios({
               method:"GET",
               url:`https://api.flightstats.com/flex/airports/rest/v1/json/withinRadius/${resp.data.location.lng}/${resp.data.location.lat}/50?appId=a7c23de7&appKey=2f302012c2e552475508bee87ebb20bd`
             }).then(async finres=>{
              try {
                 await axios({
                  method:"GET",
